...
object ObjectSchema {
  def apply(value: Map[String, JsonSchema[_]]): ObjectSchema = {
    ObjectSchema(SchemaProperties(
      ObjectTypesProperty()
    ).merge(value))
  }
}

case class ObjectSchema(override val properties: SchemaProperties[Map[String, JsonSchema[_]]] = SchemaProperties.empty) extends JsonSchema[Map[String, JsonSchema[_]]] {
...
case class ObjectTypesProperty(objectTypes: Map[String, JsonSchema[_]] = Map.empty[String, JsonSchema[_]]) extends SchemaProperty[Map[String, JsonSchema[_]]] {
  ...
  override def merge(value: Map[String, JsonSchema[_]]) = {
    ...
    ObjectTypesProperty(grouped.view.mapValues(_.map(_._2).reduce(_.merge(_))).toMap)
  }
}
